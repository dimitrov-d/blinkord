...
function ServerForm({
  ...
  const handleInputChange = (
    field: keyof Omit<ServerFormData, 'id' | 'roles'>,
    value: string
  ) => {
              ...
              >
                ...
                <div className="flex items-center">
                  <MotionInput
                    ...
                    onChange={(e) => handleDiscordRolePriceChange(DiscordRole.id, e.target.value)}
                    className="w-32 mr-2"
            ...
            <Suspense fallback={<BlinkCardSkeleton />}>
              {isLoading ? (
                ...
                >
                  <Blink
                    action={{
                      title: formData.name || "Your Server Name",
                      description: formData.description || "Your Server Description",
                      fields: DiscordRoles.filter(DiscordRole => DiscordRole.enabled).map((DiscordRole) => `${DiscordRole.name} - ${DiscordRole.price} SOL`),
                    }}
                    websiteText={formData.name || "Your Discord Server"}
                  />
                </motion.div>
              )}
            </Suspense>
